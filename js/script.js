...
function validation(event) {
  ...
  if(monsterDescription.value.length > 128){
    errorMessages.push("La description du monstre ne doit pas dépasser 128 caractères");
  }
...
function create() {
  ...
  let monsterBackground = document.querySelector("input[name='monsterBackground']:checked");
  if(nbrOfMonsters == 5){
    return}
  nbrOfMonsters++;
...
card.innerHTML+='<img src="img/realMonsters/'+monsterType.value+'.png"/>';
...
//event.target.value = parseInt(event.target.value);
  ...
  let points = parseInt(monsterAttack.value) + parseInt(monsterDefense.value) + parseInt(monsterHeatlh.value);
  if(points>10){
